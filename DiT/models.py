...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, current, cache_dic):
        ...
        if current['type'] == 'full':  # Force Activation: Compute all tokens and save them in cache
            ...
            if cache_dic['PRIOR_ERROR_MODELING'] and current['layer'] == current['num_layers'] - 1:
                with torch.no_grad():
                    if current['step']>=current['num_steps']-cache_dic['PEM_C']:
                        cache_dic['prior_cache'].append(x.mean(dim=1).detach())
                    else:
                        # mean at token level to reduce VRAM usage
                        # when PEM, batch_size = 1
                        cache_feat = cache_dic['prior_cache'].popleft()
                        curre_feat = x.mean(dim=1).detach()
                        cos_sim = torch.dot(cache_feat.reshape(-1), curre_feat.reshape(-1)) / (
                            cache_feat.norm() * curre_feat.norm() + 1e-8
                        )
                        cache_dic['prior_errors'][current['step']] = 1.0 - cos_sim
                        cache_dic['prior_cache'].append(curre_feat)
                    # PEM finished
                    if current['step'] == 0:
                        # print(cache_dic['prior_errors'])
                        os.makedirs(cache_dic['prior_path'], exist_ok=True)
                        torch.save(cache_dic['prior_errors'].detach().cpu(), f"{cache_dic['prior_path']}/{cache_dic['sample_id']}.pt")
